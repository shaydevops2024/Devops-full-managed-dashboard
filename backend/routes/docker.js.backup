...
// Delete a container
router.delete('/containers/:id', auth, async (req, res) => {
  try {
    const { id } = req.params;
    
    logger.info(`Attempting to delete container: ${id}`);
    ...
    try {
      ...
      if (containerInfo.State.Running) {
        return res.status(400).json({ 
          success: false,
          error: 'Cannot delete a running container. Please stop it first.' 
        });
      }
    } catch (error) {
    ...
    logger.info(`Container deleted successfully: ${id}`);
